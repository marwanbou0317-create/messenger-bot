...
async function loginHelper(credentials, globalOptions, callback, setOptionsFunc, buildAPIFunc, initialApi, fbLinkFunc, errorRetrievingMsg) {
    ...
    try {
        ...
        // Save critical cookies before GET request (Facebook may clear them on redirect)
        const criticalKeys = ['c_user', 'xs', 'fr', 'sb', 'datr'];
        const savedCookies = {};
        if (appState && Array.isArray(appState)) {
            for (const c of appState) {
                const key = c.key || c.name;
                if (criticalKeys.includes(key)) savedCookies[key] = c.value;
            }
        }
        // Store fallback userID in globalOptions for buildAPI
        if (savedCookies['c_user']) globalOptions._cUser = savedCookies['c_user'];

        const resp = await utils.get(fbLinkFunc(), jar, null, globalOptions, { noRef: true }).then(utils.saveCookies(jar));

        // Restore critical cookies if cleared by Facebook response
        const domain = ".facebook.com";
        const expires = new Date().getTime() + 1000 * 60 * 60 * 24 * 365;
        const jarCookiesAfter = jar.getCookiesSync(`https://${domain}`);
        for (const [key, value] of Object.entries(savedCookies)) {
            const found = jarCookiesAfter.find(c => c.cookieString().startsWith(key + '='));
            if (!found) {
                jar.setCookie(`${key}=${value}; expires=${expires}; domain=${domain}; path=/;`, `https://${domain}`);
            }
        }
